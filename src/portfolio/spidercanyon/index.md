<style type="text/css" rel="stylesheet">

img{
    width: 40%;
    height: 40%;
}


</style>

# Spider Canyon

### [Steam Page](https://store.steampowered.com/app/3451200/Spider_Canyon/)

---

## About

_Spider Canyon_ is a fast-paced puzzle platformer that challenges players to navigate a perilous environment through precise control and mastery of dynamic movement. It was originally made for a _Mini Jam 175: Gravity_ but after submitting me and my jam partner saw the potential in it and an oppourtunity to properly release a game on Steam.

![](Media/sc.gif) 


---

## Project Info

Project Type: Indie

Time on Project: 6 Months

Engine Used: Godot

---

## What I Did

Since this project was a joint venture, I was the lead developer on this project while my partner focused on Art, Level Design, and Testing. Alot of the programming effort went into refining the systems that were made for the jam and implementing new systems for it to be suitable for release. 

* Refining the First Person Controller
* Fixing Bugs
* Load/Save system
* Creating tools to aid in Level Development
* UI and Options menu
* Sound Effects
* Visual Effects

A big chunk of my work also went into the visuals of the game. The game underwent a *huge* visual overhaul from the itch.io release and the final release. This was probably the single biggest change, along with the First Person Controller, and lots of tweaking went into both to get it _just right_. 

---

![](Media/sc_after.jpg)

---

